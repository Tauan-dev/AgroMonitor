...
async def analyze(input_data: ModelInput):
    try:
        ...
        sens_user = sensitivity_analysis(A_eq, b_eq, input_data.rel_perturb)
        x_pert_user = sens_user["x_pert_b"]
        total_profit_pert = float(profit @ x_pert_user)

        # Sensibilidade FIXA (para gráficos comparativos)
        FIXED_PERTURB = 0.05
        sens_fixed = sensitivity_analysis(A_eq, b_eq, FIXED_PERTURB)
        ...
        comparison_img = plot_base_vs_perturbed(
            ...
            sens_fixed["x_pert_b"],
            input_data.crops,
            fixed_perturb=FIXED_PERTURB        # Label perturbação fixa
        )
        ...
        return {
            ...
            "profit_pert": total_profit_pert,
            "rel_dx": float(sens_user["rel_dx"]),
            "rel_db": float(sens_user["rel_db"]),
            "bound": float(sens_user["bound"]),
            "heatmap": heatmap_img,
            "comparison": comparison_img,           # Gráfico agora compara base vs perturbado para 5% FIXO
